...
type dbStruct struct {
	...
	genFullTableName string   //全路径名
	genDbName        string   //DbName方法名，
...
func (ds *dbStruct) GenFullTableName(v string) *dbStruct {
	ds.genFullTableName = v
	return ds
}

func (ds *dbStruct) GenDbName(v string) *dbStruct {
	ds.genDbName = v
	return ds
}
...
func (ds *dbStruct) Generate() (err error) {
	...
	if ds.singleFile {
		ds.modelPath += fmt.Sprintf("/model/%s.go", ds.dbName)
		ds.packageName = "model"
	} else {
		ds.modelPath += fmt.Sprintf("/model/%s/", ds.dbName)
		ds.packageName = ds.dbName
	}
	...
	for _, table := range tables {
		go ds.genStruct(table, gch, group)
	}
...
func (ds *dbStruct) genStruct(table Table, ch chan *genStructRes, group *sync.WaitGroup) {
	defer group.Done()
	buffer := bytes.Buffer{}
	res := &genStructRes{"", "", nil, false}
	structName := ds.getFormatName(table.TableName, ds.structNameFmt)
	...
	for _, column := range table.Cols {
		...
		if ds.tags != nil && len(ds.tags) > 0 {
			...
			//添加desc注释字段,移除注释行 toney
			tagString += fmt.Sprintf("%s:\"%s\"", "desc", column.Comment)
	...
	if ds.genTableName != "" {
		buffer.WriteString(fmt.Sprintf("func (%s *%s) %s() string {\n\treturn \"%s\"\n}", strings.ToLower(structName[0:1]),
			structName, ds.genTableName, table.TableName))
	}
	buffer.WriteString("\n")
	if ds.genDbName != "" {
		buffer.WriteString(fmt.Sprintf("func (%s *%s) %s() string {\n\treturn \"%s\"\n}", strings.ToLower(structName[0:1]),
			structName, ds.genDbName, table.DbName))
	}
	buffer.WriteString("\n")
	if ds.genFullTableName != "" {
		buffer.WriteString(fmt.Sprintf("func (%s *%s) %s() string {\n\treturn \"%s.%s\"\n}", strings.ToLower(structName[0:1]),
			structName, ds.genFullTableName, table.DbName, table.TableName))
	}

	//生成全局变量
	buffer.WriteString("\n")
	buffer.WriteString(fmt.Sprintf("var Tb%s = &%s{}", structName, structName))
...
func (ds *dbStruct) getTables() (tables map[string]Table, err error) {
	tableIn := ""
	...
	//填充表注释
	sqlStr := fmt.Sprintf("SELECT TABLE_NAME,TABLE_COMMENT FROM information_schema.TABLES WHERE table_schema='%s';", ds.dbName)
	tableRows, _ := ds.db.Query(sqlStr)
	defer func() {
		qerr := tableRows.Close()
		if qerr != nil {
			log.Fatalf("关闭数据查询结果异常：%s", qerr.Error())
		}
	}()
	...
	for tableRows.Next() {
		...
		err := tableRows.Scan(&tname, &tcomment)
		if err != nil {
			return nil, err
		}
		t, has := tables[tname]
