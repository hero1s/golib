...
func TestMgo(t *testing.T) {
	...
	err := Insert(database, collection, data)
	if err != nil {
	...
	var result Data
	err = FindOne(database, collection, bson.M{"_id": bson.ObjectIdHex("5b3db2334d661ff46ee14b9c")}, nil, &result)
	fmt.Println("find one with all fields", result)

	// find one without id field
	var result1 Data
	err = FindOne(database, collection, bson.M{"_id": bson.ObjectIdHex("5b3db2334d661ff46ee14b9c")}, bson.M{"_id": 0}, &result1)
	fmt.Println("find one without id field", result1)

	//find all documents
	var allResult []Data
	err = FindAll(database, collection, nil, nil, &allResult)
	fmt.Println("find all docs", allResult)

	// find all documents with query and selector
	var allResult1 []Data
	err = FindAll(database, collection, bson.M{"title": "博客的标题 1"}, bson.M{"_id": 0}, &allResult1)
	...
	//find the cursor
	var iterAll []Data
	iter := FindIter(database, collection, nil)
	err = iter.All(&iterAll)
	fmt.Println("find cursor ", iterAll)

	//update one document
	err = Update(database, collection, bson.M{"_id": bson.ObjectIdHex("5b3db2334d661ff46ee14b9c")}, bson.M{"$set": bson.M{
		"title": "更新后的标题",
		"des":   "更新后的描述信息",
		"date":  time.Now(),
	}})
	...
	//update all docments
	/*err = UpdateAll(database, collection, nil, bson.M{"$set": bson.M{
		"title": "更新所有的标题",
		"date":  time.Now(),
	}})
	if err != nil {
	...
	//delete one docment
	err = Remove(database, collection, bson.M{"_id": bson.ObjectIdHex("5b3db2334d661ff46ee14b99")})
	if err != nil {
		fmt.Println("remove one doc error", err)
	}

	//upsert the docment
	err = Upsert(database, collection, bson.M{"title": "Title Upsert"}, bson.M{"$set": bson.M{
		"des":     "描述Upsert",
		"date":    time.Now(),
		"Content": "内容Upsert",
	}})
	...
	insertResult, _ := BulkInsert(database, collection, d1, d2)
	...
	updateResult, _ := BulkUpdate(database, collection, up1, up2, up3, up4)
	...
	updateAllResult, _ := BulkUpdateAll(database, collection, up5, up6)
